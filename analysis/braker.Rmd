...
echo "fasterq-dump and gzip completed successfully."
  ...
  4. Prepare the BAM file with MiniMap2 and Samtools
  ```{r, eval=FALSE}
  minimap2 -t48 -ax splice:hq -uf genome.fa isoseq.fa > isoseq.sam
  Samtools view -bS --threads 48 isoseq.sam -o isoseq.bam
  ```
  
  5. Build the Singularity image
  ```{r, eval=FALSE}
  singularity build braker3_lr.sif docker://teambraker/braker3:isoseq
  ```
  
  6. Call BRAKER3 with the BAM file
  ```{r, eval=FALSE}
