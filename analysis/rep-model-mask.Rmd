...
# genome sizes
genome.sizes <- tibble(
  sample = c('Schistocerca piceifrons', 'Schistocerca cancellata', 'Schistocerca nitens', 'Schistocerca serialis cubense', 'Anabrus simplex - Idaho', 'Anabrus simplex - Colorado'),
  size = c(8700000000, 8500000000, 8800000000, 9100000000, 6400000000, 6900000000)
)
...
df.kimura <- fs::dir_ls(
  ...
  mutate( # ------------------------------------------------ EDIT HERE -------------------------------------------- #
    sample = case_when(
      ...
      sample == 'song_assembly/SchCanc.distance' ~ 'Schistocera cancellata',
      ...
      sample == 'song_assembly/AnsimpID.distance' ~ 'Anabrus simplex - Idaho',
      sample == 'song_assembly/AnsimpCO.distance' ~ 'Anabrus simplex - Colorado',
      .default = sample
    )
  ) |>
  select(-starts_with('...')) |>
  ...
  mutate(
    ...
    sample = factor(sample, levels = c('Schistocerca piceifrons', 'Schistocerca cancellata', 'Schistocerca nitens', 'Schistocerca serialis cubense', 'Anabrus simplex - Idaho', 'Anabrus simplex - Colorado'))
    ) |> # -------------------------------------------------------------------------------------------------------- #
...
# Save PNG
pdf(
  file = here('multigenome-kimura.pdf'),
  width = 20,
  height = 10
)
print(kimura.gg)
