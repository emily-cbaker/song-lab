...
# Data for the table
data <- data.frame(
  Column_1 = c("Translated Coding Sequence URL (_translated_cds.faa.gz)"),
  Column_2 = c("Protein Sequence URL (_protein.faa.gz)"),
  Column_3 = c("GFF Annotation (_genomic.gff.gz)"),
  Column_4 = c("Species abbreviation code"),
  Column_5 = c("Longest isoform? (yes/no)")
)
...
and so on. Be sure to copy and paste the very last line of your .csv file and paste it below what you want to be your last line. There is a quirk in this pipeline where it stops at the second to last line, so to ensure all of our data is loaded correctly, make sure you add in this line.
...
First, we need to load the the necessary modules onto your terminal. The commands are as follows:
...
**Is orthologr not available?** Copy and paste the code from [this link](https://github.com/drostlab/orthologr) under *Install orthologr*. <mark> Use this code in the RStudio web app on Sol, as it will not work on an ssh login</mark>
...
For this next step we will need to create a mamba envrionment as well as alter one of Barkdull's files with some of our own lines.

###### Mamba environment

Use the following code to create a new environment called *ortho-step3*.
...
mamba create -n ortho-step3 -c conda-forge -c bioconda python=3 TransDecoder 
source activate ortho-step3
mamba install BioPython
source deactivate
```

This creates a mamba environment with TransDecoder and BioPython installed, which are necessary for this next step.
