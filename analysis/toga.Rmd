...
3. A chain file containing chains of co-linear local alignments between the reference and query genome. Learn about how to generate these files [here](mlc.html).  
...
###### Locally
 To run TOGA locally, use the following command:

```{r launch-toga-local, eval=FALSE}
toga.py test_input/hg38.mm10.chr11.chain test_input/hg38.genCode27.chr11.bed test_input/hg38.2bit test_input/mm10.2bit --kt --pn test -i supply/hg38.wgEncodeGencodeCompV34.isoforms.txt --cb 3,5 --cjn 500 --u12 supply/hg38.U12sites.tsv --ms --nextflow_dir nextflow_logs
...
Then to run TOGA use the command
...
2. Install the  necessary utilities. Some of these are necessary to run the pipeline and some will be used to convert our fasta files to the correct formats. You can download all of them using this command:
...
#### Permission denied for chain files in modules
  I believe this is another error with the new TOGA installation in the cluster, I have contacted IT again.
