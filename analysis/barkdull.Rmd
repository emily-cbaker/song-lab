...
David Emms says that sometimes when running OrthoFinder with IQ-Tree, it will randomly throw an error when in the tree inference step. The way to correct this is to manually root the tree using the following script:
...
##NECESSARY JOB SPECIFICATIONS
#SBATCH --job-name=manual-root-tree-orthofinder         #Set the job name to "JobExample4"
#SBATCH --time=06:00:00         #Set the wall clock limit to 1hr and 30min
#SBATCH --ntasks=2              #Request 1 task
#SBATCH --cpus-per-task=4       #Request 1 task
#SBATCH --mem=30G              #Request 100GB per node
...
This will pick up from where your run quit, root the tree, and then run OrthoFinder with the tree generated.
